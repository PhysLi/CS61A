...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    valid_paras = [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        return 100.0
    elif (len(source_words) == 0) ^ (len(typed_words) == 0):
        return 0.0
    else:
        accurate_words = len(typed_words)
        for i in range(len(typed_words)):
            if i >= len(source_words):
                break
            elif typed_words[i] != source_words[i]:
                accurate_words -= 1
        accurate_words -= (len(typed_words) - len(source_words) if len(typed_words) - len(source_words) > 0 else 0)
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        if (typed, source) in list(cache) and limit <= cache[(typed, source)][1]:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for source in word_list:
        if typed_word == source:
            final_word = typed_word
            break
        diff = diff_function(typed_word, source, limit)
        if diff < diff_min:
            final_word, diff_min = source, diff
    if diff_min > limit:
        final_word = typed_word
    return final_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0:
        ...
        add = (1 if typed[0] != source[0] else 0)
        return add + furry_fixes(typed[1:], source[1:], limit - add)
    # END PROBLEM 6
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        fastest_player, fastest_time = -1, 9999
        for j in player_indices:
            player_time = get_time(times, j, i)
            if player_time < fastest_time:
                fastest_player, fastest_time = j, player_time
        fastest_words[fastest_player].append(words[i])
