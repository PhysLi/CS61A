...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        operator = env.lookup(expr.first)
        operands = expr.rest.map(lambda sub_expr: scheme_eval(sub_expr, env))
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        for i in range(len(args)):
            args_list.append(current_pair.first)
...
def eval_all(expressions, env):
    ...
    while sub_expression.rest is not nil:
        scheme_eval(sub_expression.first, env)
        sub_expression = sub_expression.rest
    return scheme_eval(sub_expression.first, env)
