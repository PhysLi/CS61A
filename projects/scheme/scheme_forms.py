...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        Lambda = LambdaProcedure(formals, body, env)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            return test if res is None else res
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    sub_binding = bindings
    while sub_binding is not nil:
        names = Pair(sub_binding.first.first,names)
        vals = Pair(scheme_eval(sub_binding.first.rest.first, env), vals)
        sub_binding = sub_binding.rest
    # END PROBLEM 14
