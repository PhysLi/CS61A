...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.stock == 0:
            ...
            self.balance = 0
            self.stock -= 1
            return f'Here is your {self.product} and ${balance - self.price} change.'
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(k, tail_link = Link.empty):
        if k == 0:
            return tail_link
        else:
            last, rest = k % 10, k // 10
            return helper(rest, Link(last, tail_link))
    return helper(n)
...
def deep_map_mut(func, s):
    ...
    """
    "*** YOUR CODE HERE ***"
    if isinstance(s.first, Link) == False:
