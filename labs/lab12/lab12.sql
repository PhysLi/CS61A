...
CREATE TABLE sharing AS
  SELECT a.course, count(DISTINCT a.hall) FROM finals AS a, finals AS b WHERE a.hall = b.hall and a.course != b.course GROUP BY a.course ORDER BY count(DISTINCT a.course) ;
...
CREATE TABLE remaining AS
  SELECT course, sum(seats) - max(seats) FROM finals, sizes WHERE hall = room GROUP BY course;
